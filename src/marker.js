...
const activity = coordinates => {
  let markerDiv = document.createElement('div');
  markerDiv.style.width = '255px';
  markerDiv.style.height = '170px';
  ...
  new mapboxgl.Marker(markerDiv).setLngLat([-87.641, 41.895]).addTo(map);
};

const hotel = coordinates => {
  let markerDiv = document.createElement('div');
  markerDiv.style.width = '32px';
  markerDiv.style.height = '39px';
  markerDiv.style.backgroundImage = 'http://i.imgur.com/D9574Cu.png';
  new mapboxgl.Marker(markerDiv).setLngLat(coordinates).addTo(map);
};
